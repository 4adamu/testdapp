...
function Nav({ page, setPage }) {
  ...
  const routes = ['home','preview','airdrop','liquidity','docs']
  ...
  return (
    <div className="wrap page-section grid-bg">
      ...
      <div className="cols-12" style={{ alignItems:'end' }}>
        <div className="col-7">
          ...
          <p className="mt-8 op-80" style={{ maxWidth:560, lineHeight:1.6 }}>
            Every swap in the <span className="c-white">ELING/ETH</span> pool summons a 32×32 pixel spirit — generated and stored entirely on-chain by a Uniswap v4 hook. No sale. No VCs. Just a free airdrop for the community and dev-seeded liquidity.
          </p>
          <div className="flex-wrap-gap mt-8">
            ...
            <button className="btn" onClick={() => setPage('liquidity')}>Liquidity</button>
          </div>
          <div className="mt-8">
            <span className="chip">We Inspired By UniPeg</span>
            <span className="chip">Experiment project</span>
            <span className="chip">No sale · airdrop only</span>
      ...
      <div className="strip mt-16">
        ...
        <div><div className="tag">No sale</div><div className="strip-v">Free</div><div className="strip-s">airdrop only · no presale · no VC</div></div>
        <div><div className="tag">Airdrop</div><div className="strip-v">1 <span style={{fontSize:20,opacity:.6}}>per wallet</span></div><div className="strip-s">FCFS · max reach · every eligible wallet</div></div>
        <div><div className="tag">LP seeded by</div><div className="strip-v">Dev</div><div className="strip-s">locked on UniCrypt until 2030</div></div>
      </div>
...
function AirdropPage() {
  ...
  const rem  = dropState?.remaining ?? 10000n * 10n**18n
  const remN = Number(rem) / 1e18
  const slots = Math.floor(remN)   // 1 token per wallet = slots = remN
  const pct   = Math.min(100, (remN / 10000) * 100)
  ...
  const statusLabel = () => {
    ...
    if (eligResult.status === 'eligible')          return <span className="c-lime">✓ Eligible — {eligResult.path === 'merkle' ? 'uPEG holder' : 'v4 hook user'} · claim 1 $ELING</span>
  ...
  return (
    <div className="wrap page-section">
      ...
      <h1 className="hero-title mt-3">1 per wallet.<br/><span className="alt">reach everyone<span className="c-lime">.</span></span></h1>
      <p className="mt-6 op-80 sm" style={{ maxWidth:720, lineHeight:1.7 }}>
        No sale, no whitelist games. If your wallet held <span className="c-white">uPEG</span> at snapshot or ever used a Uniswap v4 hook, you get exactly <span className="c-white">1 $ELING</span>. One token per wallet, maximum reach, first come first served. Unclaimed tokens go to a community-voted destination.
      </p>
      <div className="cols-12 mt-8">
        <div className="col-7 card card-lg">
          <div className="flex-between" style={{ alignItems:'flex-start' }}>
            <div>
              ...
              <div style={{ fontFamily:'var(--font-display)', fontSize:36, marginTop:4 }}>
                {Math.floor(remN).toLocaleString()} <span className="op-40" style={{ fontSize:18 }}>wallets remaining</span>
              </div>
            </div>
          ...
          <div style={{ marginTop:24 }}>
            ...
            <div className="flex-wrap-gap mt-5">
              ...
              >
                {claiming ? 'Confirm in MetaMask...' : '▸ Claim 1 $ELING'}
              </button>
            </div>
          </div>
        </div>
        <div className="col-5 card card-lg">
          ...
          <p className="xs op-60" style={{ marginTop:20, borderTop:'1px solid var(--hair)', paddingTop:12, lineHeight:1.7 }}>
            1 $ELING per wallet keeps it fair and maximises how many people hold the token at launch. Unclaimed tokens after the deadline go to a destination chosen by community vote — not kept by the team.
          </p>
        </div>
      </div>
    </div>
...
function LiquidityPage() {
  ...
  const steps = [
    ['Dev seeds liquidity', 'The team funds the initial ELING/ETH liquidity from the dev pocket — ETH from the team paired with the $ELING liquidity allocation. No public sale, no fundraise.'],
    ['Mint full-range v4 LP', 'Tokens + ETH deposited as a full-range position in the ELING/ETH v4 pool. Full-range so the hook fires for every swap regardless of price movement.'],
    ['Lock LP NFT on UniCrypt', 'Unlock time: 1893456000 (Jan 1 2030 UTC). The lock proof URL is posted in the footer immediately after locking.'],
    ['Renounce ownership', 'eling.renounceOwnership(). Supply is permanently fixed, no admin keys remain, contract is fully immutable.'],
  ]
  return (
    <div className="wrap page-section">
      <div className="tag">§07 — liquidity</div>
      <h1 className="hero-title mt-3">dev seeds.<br/><span className="alt">community keeps<span className="c-magenta">.</span></span></h1>
      <div className="cols-12 mt-8">
        <div className="col-7 card card-lg">
        ...
        <div className="col-5 card card-lg">
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10, marginTop:16 }}>
            {[['Pair','$ELING / ETH'],['Pool fee','0.30%'],['Range','Full range'],['Funded by','Dev team pocket'],['Sale','None — no public sale'],['Lock venue','UniCrypt'],['Unlock','Jan 1, 2030 UTC']].map(([k,v]) => (
  ...
  const folds = [
    ...
    ['Contract addresses', false, (
      <div style={{ fontFamily:'var(--font-mono)', fontSize:12 }}>
        {[['ELING', CONFIG.ELING_ADDRESS],['Airdrop', CONFIG.AIRDROP_ADDRESS]].map(([k,v]) => (
  ...
  return (
    <div className="wrap page-section" style={{ maxWidth:960 }}>
      ...
      <article style={{ marginTop:32, fontSize:15, lineHeight:1.85, opacity:.9 }}>
        ...
        <p style={{ marginTop:16 }}>There is <b>no token sale</b>. The team seeds the initial liquidity from their own pocket and locks it until 2030. The entire community distribution is via airdrop — <b>1 $ELING per eligible wallet</b> — so the token reaches as many hands as possible from day one rather than concentrating at launch price.</p>
        ...
        <div className="footer-grid">
          <div>
            ...
            <div className="xs op-70" style={{ marginTop:20, lineHeight:1.8 }}>
              ...
              <div>🪂 No sale — 1 $ELING airdrop per wallet.</div>
              <div>💧 Liquidity seeded by dev team.</div>
            </div>
          </div>
          <div>
            <div className="tag">Navigate</div>
            <ul>
              {['home','preview','airdrop','liquidity','docs'].map(r => (
...
export default function Page() {
  ...
  const pages = { home: HomePage, preview: PreviewPage, airdrop: AirdropPage, liquidity: LiquidityPage, docs: DocsPage }
    ...
    <>
      ...
      <div className="marquee-wrap">
        <div className="marquee">
          <span>★ We Inspired By UniPeg ★ It's an Experiment Project ★ No Sale · Airdrop Only · 1 Per Wallet ★ Dev-Seeded Liquidity · Locked on UniCrypt until 2030 ★ 10,000 $ELING ★ 32×32 on-chain SVG ★ Uniswap v4 hook ★</span>
          <span>★ We Inspired By UniPeg ★ It's an Experiment Project ★ No Sale · Airdrop Only · 1 Per Wallet ★ Dev-Seeded Liquidity · Locked on UniCrypt until 2030 ★ 10,000 $ELING ★ 32×32 on-chain SVG ★ Uniswap v4 hook ★</span>
        </div>
      </div>
